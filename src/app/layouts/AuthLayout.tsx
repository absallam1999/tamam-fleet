...
const routeContent: Record<string, RouteContent> = {
  "/auth/login": {
    ...
    descriptionAr: "أدخل رقم هاتفك وكلمة المرور للدخول إلى منصة تحكم المشرف.",
    descriptionEn:
      "Enter your phone number and password to access your supervisor dashboard.",
    icon: (isAr: boolean) => (
      <svg
  ...
  "/auth/reset-password": {
    ...
    descriptionAr: "اختر كلمة مرور قوية وآمنة لحماية حساب المشرف الخاص بك.",
    descriptionEn:
      "Choose a strong, secure password to protect your supervisor account.",
    icon: (_isAr: boolean) => (
      <svg
        className="w-6 h-6"
        fill="none"
...
const features = [
  {
    ...
    descriptionAr: "مراقبة السائقين والمركبات",
    descriptionEn: "Monitor drivers & vehicles",
    icon: (
      <svg
        className="w-5 h-5"
        fill="none"
  ...
  },
  {
    ...
    descriptionAr: "تحكم كامل بالأسطول",
    descriptionEn: "Full fleet control",
    icon: (
      <svg
        className="w-5 h-5"
        fill="none"
  ...
  },
  {
    ...
    descriptionAr: "تحليلات أداء السائقين",
    descriptionEn: "Driver performance analytics",
    icon: (
      <svg
        className="w-5 h-5"
        fill="none"
...
const getLastSession = async (): Promise<LastSession | null> => {
  try {
    ...
    // const response = await fleetApi.getLastSession();
    // if (response) return mapSessionDto(response);

    // 2) Fallback to localStorage for now
    const saved = localStorage.getItem("fleet-last-session");
...
export const saveLastSession = (session: LastSession): void => {
  try {
    localStorage.setItem("fleet-last-session", JSON.stringify(session));
  } catch {
...
const AuthFooter: React.FC<{ isAr: boolean }> = ({ isAr }) => {
  return (
...
export const AuthLayout: React.FC = () => {
  ...
  const content = routeContent[location.pathname] || {
    taglineAr: "لوحة تحكم المشرف",
    taglineEn: "Fleet Supervisor Dashboard",
    descriptionAr: "تحكم كامل في أسطول المركبات والسائقين من مكان واحد.",
    descriptionEn:
      "Complete control over your fleet of vehicles and drivers from one place.",
    icon: () => (
      <svg
        className="w-6 h-6"
        fill="none"
      ...
      >
        ...
        <div
          className={cn(
            ...
            isDark
              ...
              : "bg-gradient-to-br from-primary-600 via-primary-700 to-primary-600",
          )}
        />
        ...
        <div
          className={cn(
            ...
            isDark ? "bg-primary-800/15" : "bg-primary-300/12",
          )}
          aria-hidden="true"
        />
          ...
          <div className="flex items-center gap-5 mb-12">
            ...
            <div className={isAr ? "text-right" : "text-left"}>
              ...
              <p className="text-xs xl:text-sm text-white/55 mt-1.5 font-medium tracking-wider uppercase">
                {isAr ? "منصة تحكم المشرف" : "Fleet Management Platform"}
              </p>
            </div>
          </div>
        ...
        <div className="lg:hidden w-full max-w-md mb-8 mt-12 text-center">
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-primary-100 dark:bg-primary-900/40 flex items-center justify-center">
              <img src="/icon.svg" alt="Tamam Fleet Logo" className="w-6 h-6" />
            </div>
